...
{
    /**
     * Show the full page project chat view (Inertia)
     */
    public function index(Request $request)
    {
        ...
        // Build query
        $query = Project::with([
            'projectTeamMembers.user.media' => function($q) {
                $q->where('category', 'profile')->latest()->limit(1);
            }
        ]);
        ...
        $projects = $query->orderBy('created_at', 'desc')
            ...
            ->map(function ($project) use ($userId) {
                ...
                $unreadQuery = ProjectChat::where('project_id', $project->id);
                if ($lastRead) {
                    $unreadQuery->where('created_at', '>', $lastRead->last_read_at);
                }
                
                $project->unread_count = $unreadQuery->where('user_id', '!=', $userId)->count();
                return $project;
            });
        
        // Get project counts for all statuses
        $projectCounts = [
            ...
            'Total' => Project::where('project_status', '!=', 'Deliver')->count(),
        ];
        
        // Get logged in user with all required data
        $user = Auth::user();
        $user->load('media');
        
        return inertia('Pages/Chat/ProjectChat', [
            'projects' => $projects,
     ...
     */
    public function getProjectsJson(Request $request)
    {
        $userId = Auth::id();
        $status = $request->get('status', 'All');
